...
const ListServersMain = ({ servers, allservers }) => {
  const [modelOpen, setModelOpen] = useState(false);
    ...
    const handleSearch = () => {
      if (searchInput.trim() === '') {
        ...
        const filtered = allservers.filter(server => server.category.toLowerCase() === searchInput.toLowerCase());
  ...
  return (
    <>
      ...
      <Input type="text" className="px-3 py-2 w-80" placeholder="Search..." value={searchInput}
        onChange={(e) => setSearchInput(e.target.value)} />
      <Button className="px-3 py-2" onClick={handleSearch}>Search</Button>
    </div>
